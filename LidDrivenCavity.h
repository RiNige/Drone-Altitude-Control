...
class LidDrivenCavity{
    ...
    void SetReynoldsNumber(double Re);*/
        ...
        double** valueoutput_v();
        double** valueoutput_s();
...
void LidDrivenCavity::CreateMatrix(){
    ...
    for(int i = 0; i < Ny; i++){                    // intialise both arrays by initial condition(0)
        for(int j = 0; j < Nx; j++){
            v[i][j] = 0;
            s[i][j] = double (rand())*100/RAND_MAX;
        }
    }
}
...
void LidDrivenCavity::currentOmegaInt(int* pos, int* coord,MPI_Comm comm){
    ...
    // creating maximum eight buffer for top ,bot,left,right
...
double** LidDrivenCavity::valueoutput_v(){
    return &(v[0]) ;
}

double** LidDrivenCavity::valueoutput_s(){
    return &(s[0]) ;
}
